    ...
    for file in files:
        ...
        for words in words_list:
            ...
            for i in range(len_words):
                word_i = words[i]
                if (not word_i) or ('PU' in word_i) or ('(' not in word_i):
                    continue
                ...
                for j in range(i + 1, j_len):
                    value = 1 / (j - i)
                    word_j = words[j]
                    if (not word_j) or ('PU' in word_j) or ('(' not in word_j):
                        continue
...
for start_id in dict_relationship:
    ...
    for end_id in dict_relationship[start_id]:
        rel_sum += dict_relationship[start_id][end_id]
    for end_id in dict_relationship[start_id]:
        p_a_b = dict_relationship[start_id][end_id] / rel_sum
f = open('word.csv', 'w', encoding='utf-8', newline='')
csv_write = csv.writer(f)
csv_write.writerow(['word_id:ID', 'name', 'cixin', ':LABEL'])
for temp in dict_wordid:
    try:
        word, cixin = temp.split('(')
        cixin = cixin.replace(')', '')
    except:
        word = temp
        cixin = 'error'
    csv_write.writerow([dict_wordid[temp], word, cixin, 'words'])
f.close()
f = open('relationship.csv', 'w', encoding='utf-8', newline='')
csv_write = csv.writer(f)
...
for start_id in dict_relationship:
    for end_id in dict_relationship[start_id]:
        ...
        csv_write.writerow([start_id, fx(dict_relationship[start_id][end_id]), end_id, 'score'])
f.close()
